...
export default function QcBasicTab() {
  ...
  //  -> cascade : selectionner un superviseur limite les enqueteurs
  // ============================================================
  ...
  // Table enqueteur -> superviseur, construite a partir des cas basiques + IA
  // Si un enqueteur apparait sous plusieurs superviseurs (rare), on prend le premier vu
  ...
  // Liste unifiee des superviseurs presents (basiques + IA)
  ...
  // -> filtree si un superviseur est selectionne (cascade)
  ...
  }, [results, aiResult, selectedSuperviseur, enqToSup])

  // Resultats filtres : chaque test conserve ses lignes,
  // mais on ne garde que celles matchant les filtres actifs
  const filteredResults = useMemo(() => {
    if (selectedSuperviseur === ALL_ENQ && selectedEnqueteur === ALL_ENQ) {
      // Vue globale : rien a filtrer
    ...
    return (results || []).map((r) => {
      ...
      const filteredLignes = isAgrege
        ? []
        : originalLignes.filter((l) => {
            // Filtre superviseur
            if (selectedSuperviseur !== ALL_ENQ) {
              if (String(l._superviseur) !== selectedSuperviseur) return false
            }
            // Filtre enqueteur
            if (selectedEnqueteur !== ALL_ENQ) {
              if (String(l._enqueteur) !== selectedEnqueteur) return false
            }
            return true
          })
  ...
  }, [results, selectedSuperviseur, selectedEnqueteur])
  ...
  // Reset : replier tout et revenir a la vue globale (les deux filtres)
  const clearFilter = () => {
  ...
  return (
    <div>
      {/* ============================================================
          BARRE DE FILTRE : SUPERVISEUR (cascade) + ENQUETEUR
          ============================================================ */}
      {(uniqueSuperviseurs.length > 0 || uniqueEnqueteurs.length > 0) && (
        <div className="bg-white border border-gray-200 rounded-2xl p-4 mb-5 shadow-sm">
          <div className="flex items-center gap-3 flex-wrap">
            ...
            {/* Dropdown superviseur (masque s'il n'y en a pas) */}
            {uniqueSuperviseurs.length > 0 && (
              <div className="flex items-center gap-2">
                <label
                  htmlFor="filter-superviseur"
                  className="text-sm text-gray-700 font-semibold"
                >
                  Superviseur :
                </label>
                <select
                  id="filter-superviseur"
            ...
            {/* Dropdown enqueteur (toujours affiche si des enqueteurs existent) */}
            <div className="flex items-center gap-2">
              <label
      ...
      <div className="flex items-center justify-between flex-wrap gap-2 mb-3">
        <h5 className="font-sora font-bold text-navy m-0">
          {isFiltered
            ? `Anomalies de ${selectedEnqueteur !== ALL_ENQ ? selectedEnqueteur : "l'équipe " + selectedSuperviseur}, regroupées par type`
      ...
      {/* Cas particulier : selection sans aucune anomalie */}
        ...
        <div className="bg-green-50 border-l-4 border-green-500 rounded-r-xl p-4 mb-3 flex items-start gap-3">
          ...
          <div>
            <p className="font-sora font-bold text-green-900 m-0">
              Aucune anomalie détectée pour{" "}
              {selectedEnqueteur !== ALL_ENQ
                ? selectedEnqueteur
                : "l'équipe " + selectedSuperviseur}
            </p>
            <p className="text-sm text-green-800 mt-1 m-0">
              Aucun cas détecté dans les tests attribuables individuellement.
            </p>
          </div>
        </div>
      )}
